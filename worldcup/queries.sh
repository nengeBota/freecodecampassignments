...
echo "$($PSQL"select name from teams inner join games on teams.team_id = games.winner_id OR teams.team_id =games.opponent_id where year
...
echo "$($PSQL"select year, name from teams inner join games on teams.team_id =games.winner_id and games.round
='Final' order by year;")"
